  ...
  describe('with access rights', () => {
    ...
    describe('/commerce/product (POST)', () => {
      ...
      it('should be possible to create product', done => {
        return request(app.getHttpServer())
          ...
          .end((err, { body }) => {
            ...
            expect(body.price).toEqual(createProductRequestMock.price);
            return done();
          });
      });
      ...
      it('should respond with Bad Request when whole object is incorrect', done => {
        return request(app.getHttpServer())
          ...
          .end((err, { body }) => {
            ...
            expect(body.message[3].property).toBe('price');
            done();
          });
      });
      ...
      describe('description error', () => {
        ...
        it('should respond with error when it is empty', done => {
          const { description, ...rest } = createProductRequestMock;
          const send = { ...rest, description: 'Vestibulum id ligula porta felis euismod semper. Etiam porta sem malesuada magna mollis euismod. Sed posuere consectetur est at lobortis. Donec sed odio dui. Vestibulum id ligula porta felis euismod semper. Etiam porta sem malesuada magna m.' };
          return request(app.getHttpServer())
            .post(URL)
            .send(send)
            .set('Authorization', `Bearer ${auth.token}`)
      ...
      describe('price error', () => {
        it('should respond with error when it is undefined', done => {
          const { price, ...rest } = createProductRequestMock;
          // @ts-ignore
          const send = { ...rest, price: undefined };
          return request(app.getHttpServer())
            .post(URL)
            .send(send)
            .set('Authorization', `Bearer ${auth.token}`)
            ...
            .end((err, { body }) => {
              ...
              expect(body.error).toEqual('Request validation failed');
              expect(body.message[0].value).toEqual(undefined);
              expect(body.message[0].property).toEqual('price');
              expect(body.message[0].children).toEqual([]);
              expect(body.message[0].constraints.isDefined).toEqual(
                'price should not be null or undefined',
              );
              expect(body.message[1]).toBeUndefined();
              done();
            });
